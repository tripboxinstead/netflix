...
function getGenreList() {
    return async (dispatch) => {
        try {
            dispatch({type:"GET_MOVIE_GENRE_REQUEST"});

            const genreList = await api.get(`/genre/movie/list?api_key=${API_KEY}&language=en-US`);
            ...
            dispatch({type:"GET_MOVIE_GENRE_FAILURE"});
        }
    }
}
